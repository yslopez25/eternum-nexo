...
class SFX {
  ...
  tone({type="sine", f=440, t=0.2, a=0.01, d=0.03, s=0.5, r=0.08, gain=0.4, sweep=0}){
    this.ensure();
    const ctx=this.ctx, g=ctx.createGain(), o=ctx.createOscillator();
    o.type=type; o.frequency.value=f;
    const now=ctx.currentTime;
    if(sweep) { o.frequency.setValueAtTime(f,now); o.frequency.linearRampToValueAtTime(f+sweep, now+t*0.8); }
    const env=g.gain;
    ...
    o.start(); o.stop(now + Math.max(t, a+d+r) + 0.02);
  }
...
// Controles de audio (si existen en el HTML actual)
...
function doAction(){
  state.players.forEach(p=>{
    ...
    if(!acted && p.resources.food>=2 && owned.length){
      p.resources.food-=2; owned[rand(owned.length)].troops+=1;
...
function doNexoEvent(){
  ...
  if(roll===0){
    shuffleBoard();
    for(let i=0;i<3;i++){ const t=state.board[rand(state.board.length)]; addPulse(t.id,"#00d4ff",500); }
...
function nextPhase(){
  ...
  state.phase++;
  if(state.phase>=PHASES.length){ state.phase=0; state.turn++; checkVictory(); }
  renderAll();
}

// Turno completo (5 fases con breve pausa)
...
function drawBoard(){
  ...
  state.board.forEach(t=>{
    ...
    if(eShake){
      ...
      const amp=(1-dt)*6, ang=tNow/12;
      x+=Math.sin(ang+t.r)*amp; y+=Math.cos(ang+t.c)*amp;
    }
    ...
    grad.addColorStop(0,"#ffffff12"); grad.addColorStop(1,"#00000012");
    ctx.fillStyle=fills[t.type]||"#2a2a2a"; ctx.fill(); ctx.fillStyle=grad; ctx.fill();
    ctx.restore();

    // borde
    ctx.lineWidth=2; ctx.strokeStyle="#0b1020";
    drawRoundedRect(x,y,w,h,14); ctx.stroke();

    // glow dueño
    if(t.owner){
      const idx=Math.max(0,state.players.findIndex(p=>p.id===t.owner));
      const glow=PLAYER_COLORS[idx%PLAYER_COLORS.length];
      ctx.save(); ctx.shadowColor=glow; ctx.shadowBlur=14; ctx.lineWidth=3; ctx.strokeStyle=glow;
      drawRoundedRect(x,y,w,h,14); ctx.stroke(); ctx.restore();
    }

    // pulse
    const ePulse=effects.find(e=>e.tileId===t.id && e.kind==="pulse");
    if(ePulse){
      const dt=(tNow-ePulse.started)/ePulse.duration;
      if(dt>=0 && dt<=1){
        const alpha=0.45*(1-dt), spread=6+dt*14;
        ctx.save();
        ctx.shadowColor=ePulse.color||"#fff";
        ctx.shadowBlur=22+dt*30; ctx.lineWidth=3+dt*4; ctx.strokeStyle=`rgba(255,255,255,${alpha})`;
        drawRoundedRect(x-spread,y-spread,w+spread*2,h+spread*2,18); ctx.stroke();
        ctx.restore();
      }
    }
  ...
  if(state.selectedTileId){
    ...
    if(t){
      ...
      ctx.save();
      ctx.lineWidth = 5; ctx.strokeStyle = "#ffffffcc";
      drawRoundedRect(x-2,y-2,w+4,h+4,16); ctx.stroke();
      ctx.restore();
    }
  }
...
function drawMinimap(){
  ...
  state.board.forEach(t=>{
    ...
    if(t.owner){
      const idx=Math.max(0,state.players.findIndex(p=>p.id===t.owner));
      mctx.strokeStyle=PLAYER_COLORS[idx%PLAYER_COLORS.length];
      mctx.lineWidth=2; mctx.strokeRect(x+1,y+1,w-2,h-2);
    }
  });
  // marca de selección
  if(state.selectedTileId){
    const t=state.board.find(x=>x.id===state.selectedTileId);
    if(t){ mctx.strokeStyle="#fff"; mctx.lineWidth=2; mctx.strokeRect(t.c*cw+1, t.r*ch+1, cw-2, ch-2); }
  }
}
...
function renderSidebar(){
  ...
  if(phaseEl) phaseEl.textContent = "Fase: "+PHASES[state.phase];
  ...
  if(logEl){
    logEl.innerHTML = state.log.map(x=>{
...
function tryMoveOrAttack(from, to){
  ...
  if(Math.abs(from.r - to.r) + Math.abs(from.c - to.c) !== 1){
    log("Solo puedes actuar en islas adyacentes.");
    return;
  }
  const me = ownerOf(from);
  if(!me){ log("Selecciona primero una isla que controles."); return; }
  ...
  if(!to.owner || to.owner===from.owner){
    ...
    if(!to.owner) to.owner = from.owner;
    to.troops += 1;
    addShake(to.id, 250); sfx.move();
    log("Movimiento de 1 tropa a isla adyacente.");
  }else{
...
function endPress(px, py){
  ...
  if(dx<8 && dy<8){
    ...
    if(sel){
      tryMoveOrAttack(sel, t);
      if(sel.troops<=0) state.selectedTileId = t.id;
    }else{
      if(t.owner){
        const me = state.players[0]; // Jugador 1 como local
        if(t.owner===me.id) selectTile(t);
        else log("Esa isla no es tuya.");
      }else{
        log("Selecciona primero una isla que controles.");
      }
    }
  }
}
...
canvas.addEventListener("touchmove",  e=>{
  ...
  const t=e.touches[0];
  if(Math.abs(t.clientX-pressStartPos.px)>12 || Math.abs(t.clientY-pressStartPos.py)>12){
    clearTimeout(pressTimer); pressStartPos=null;
  }
}, {passive:true});
...
const fullBtn     = document.getElementById("playTurn");
const resetBtn    = document.getElementById("resetGame");

if(newGameBtn) newGameBtn.addEventListener("click", ()=>{ sfx.click(); newGame(3); });
if(nextBtn)    nextBtn.addEventListener("click", ()=>{ sfx.click(); nextPhase(); });
if(fullBtn)    fullBtn.addEventListener("click", ()=>{ sfx.click(); playFullTurn(); });
if(resetBtn)   resetBtn.addEventListener("click", ()=>{ sfx.click(); newGame(3); });
...
function newGame(players=3){
  gameOver=false;
  state.turn=1; state.phase=0; state.log=[]; effects.length=0; state.selectedTileId=null;
  newBoard(); newPlayers(players);
  renderAll();
